...
r_des = np.array([ \
    ...
    #0.1* np.sin(2*t),
    #np.full((2*N+1),-0.15),
    #np.linspace(-0.15, 0, 2*N+1),
    np.hstack((np.linspace(-0.15, 0, N), np.repeat(0, N+1))),
    #0.15* np.cos(2*t),
    np.full((2*N+1),1.4)
    #1.2 + 0.29*np.sin(2*t)
    ])
...
c_init = np.hstack(( \
    np.array([foot_size[0]/2, -torso_size[1]/2-foot_size[1]/2, 0])[:,None], #temp
...
c_des = np.hstack(( \
    ...
    np.array([-foot_size[0]/2, -torso_size[1]/2-foot_size[1]/2, 0])[:,None],
    np.array([-foot_size[0]/2, -torso_size[1]/2+foot_size[1]/2, 0])[:,None],
    np.array([foot_size[0]/2, torso_size[1]/2-foot_size[1]/2, 0.07])[:,None],
    np.array([foot_size[0]/2, torso_size[1]/2+foot_size[1]/2, 0.07])[:,None],
    np.array([-foot_size[0]/2, torso_size[1]/2-foot_size[1]/2, 0.07])[:,None],
    np.array([-foot_size[0]/2, torso_size[1]/2+foot_size[1]/2, 0.07])[:,None]
    ))
...
for i in range(2*N+1):
    ...
    for j_idx in range(12, nj3): # temp
        cj_des = c_des.reshape(nj3, order='F')[j_idx]
        J += Qc*simp[0][i]*(XC[j_idx,i]-cj_des)**2
        if i != 0:
            J += Qcd*simp[0][i]*(XC[j_idx,i]-XC[j_idx,i-1])**2
...
opti.subject_to(XC[:,0] ==  c_init.reshape(nj3, order='F'))
...
for i in range(2*N+1):
    ...
    opti.subject_to(ca.repmat(cs_i[2,:], 3) * Fs_i == np.zeros(Fs_i.shape))
    opti.subject_to(ca.repmat(cds_i[0,:], 3) * Fs_i == np.zeros(Fs_i.shape))
    opti.subject_to(ca.repmat(cds_i[1,:], 3) * Fs_i == np.zeros(Fs_i.shape))
    '''
    # contact constraints
    opti.subject_to(cs_i[2,4:] >= np.zeros(cs_i[2,4:].shape))
    opti.subject_to((Fs_i[2,4:] * cs_i[2,4:]) == np.zeros(cs_i[2,4:].shape))
    #opti.subject_to((Fs_i[2,4:]) == np.zeros(cs_i[2,4:].shape))
...
anim.save('point_mass_contact_bad' + '.mp4', writer=writer)
'''
